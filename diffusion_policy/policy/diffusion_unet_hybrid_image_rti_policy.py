...
class DiffusionUnetHybridImageRTIPolicy(BaseImagePolicy):
    def __init__(self, 
            shape_meta: dict,
        ...
        self.guess: torch.Tensor = None
        self.steps = steps
        print("steps", steps)
        self.has_discrete_action = has_discrete_action
        print("has_discrete_action", has_discrete_action)
    ...
    def rti_sample(self, 
        ...
        if self.steps is not None:
            timesteps = torch.from_numpy(np.array(self.steps)).to(condition_data.device)
        else:
            raise ValueError("Steps must be provided for RTI sampling.")
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        self.guess = nsample.clone().detach()
        ...
        if self.has_discrete_action:
            ...
            if c > 9:
                ...
                if c == 20: #bimanual
                    action_pred[:,:,9] = torch.where(action_pred[:,:,9]>0.0, 1.0, -1.0)
